    ...
    <div class="flex-1 flex-y relative">
        ...
        <el-table :data="tableData" v-loading="loading" class="ft-table" style="width: 100%;" height="100%"
            ...
            <el-table-column label="操作" align="left">
                <template slot-scope="scope">
                    <div>
                        <el-button type="success" icon="el-icon-circle-plus" @click="finish(scope.row.id)"
                            v-if="scope.row.status==51" :style="{'width':'90px',' margin-left':'0px'}">填写完成
                        </el-button>
                    </div>
                </template>
            </el-table-column>
        ...
        <principal-approval @close="closeCheckApproval" :id="id" v-if="CheckApproval" :isupdate=this.isupdate
            :source=this.source  :projectId=this.projectId :name=$store.getters.name></principal-approval>
        ...
        <add-approval @close="closePrincipalApproval" v-if="PrincipalApproval" :projectId=this.projectId :addOrUpdate="1" :name=$store.getters.name>
        </add-approval>
...
<script>
    ...
    import PrincipalApproval from '../components/PrincipalApproval.vue';
    ...
    export default {
        ...
        methods: {
            ...
            finish: async function (id) {
                this.$confirm('此操作会通知申请部门和规划处并结束该项目, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(async () => {
            ...
            openPrincipalApproval: function (id) {
                this.projectId = id
                this.PrincipalApproval = true;
            },
            closePrincipalApproval: function () {
                this.selected = {};
                this.PrincipalApproval = false;
                this.queryProjectList(this.currentPage);
            },
        ...
        mounted: async function () {
            console.log('authority' + this.$store.getters.authority)
            try {
                var res = await queryProjectListEstimate(
                    this.key,
                    "51",
