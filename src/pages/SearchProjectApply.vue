    ...
    <div class="flex-1 flex-y relative">
        ...
        <el-table :data="tableData" v-loading="loading" class="ft-table" style="width: 100%;" height="100%"
                ...
                <template slot-scope="scope">
                    <span>
                        <el-button type="primary" icon="el-icon-zoom-in" v-if="scope.row.establishment!=0"
                            @click="openCheckEstablishment(scope.row.establishment,scope.row.status,scope.row.id)">查看</el-button>
                    ...
                    <span>
                        <el-tooltip class="item" effect="dark" content="这个项目不存在立项申请表！请尽快添加！" placement="right-start"
                            v-if="scope.row.establishment==0">
                ...
                <template slot-scope="scope">
                    <div>
                        <el-button type="primary" icon="el-icon-zoom-in" v-if="scope.row.estimate!=0"
                            @click="openCheckEstimate(scope.row.estimate,scope.row.status)">查看</el-button>
                        <el-button type="danger" icon="el-icon-circle-close" v-if="scope.row.estimate==0"
                ...
                <template slot-scope="scope">
                    <div>
                        <el-button type="primary" icon="el-icon-zoom-in" v-if="scope.row.approval!=0"
                            @click="openCheckApproval(scope.row.approval,scope.row.status)">查看</el-button>
            ...
            <el-table-column label="操作" align="left">
                <template slot-scope="scope">
                    <div>
                        <el-button type="danger" icon="el-icon-circle-plus" @click="updateProjectStatus(scope.row.id)"
                            v-if="scope.row.status==10" :style="{'width':'90px','  margin-left':'0px'}">重新发送
                        </el-button>
                    </div>
                </template>
            </el-table-column>
        ...
        <check-establishment @close="closeCheckEstablishment" :id="id" v-if="CheckEstablishment" :isupdate=this.isupdate :projectId=this.projectId
            :source=this.source></check-establishment>
        <check-estimate @close="closeCheckEstimate" :id="id" v-if="CheckEstimate" :isupdate=true :projectId=this.projectId
            :source=this.source></check-estimate>
        <check-approval @close="closeCheckApproval" :id="id" v-if="CheckApproval" :isupdate=true :projectId=this.projectId
            :source=this.source :name=$store.getters.name></check-approval>
        ...
        <add-approval @close="closeAddApproval" v-if="AddApproval" :projectId=this.projectId :addOrUpdate="1" :name=$store.getters.name>
        </add-approval>
        ...
        methods: {
            handleCurrentChange: async function (currentPage) {
                ...
                try {
                    ...
                    var res = await queryProjectListApply(
                        this.key,
                        null,
                        currentPage,
                        this.pageSize,
            ...
            updateProjectStatus: async function (id) {
                this.$confirm('此操作会将该项目重新送至规划处进行申请, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(async () => {
            ...
            openCheckEstimate: function (id, status) {
                this.isupdate = true
                this.id = id;
            ...
            openCheckApproval: function (id, status) {
                this.isupdate = true
                this.id = id;
