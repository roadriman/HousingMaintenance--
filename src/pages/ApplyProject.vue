<template>
    <div class="flex-1 flex-y">
        ...
        <add-approval @close="closeAddApproval" v-if="AddApproval" :addOrUpdate="1" :projectId=this.projectId :name=$store.getters.name>
        </add-approval>
    </div>
</template>
...
<script>
    ...
    export default {
            ...
            verification: async function () {
                ...
                if (res.status == 1) {
                    ...
                    if (this.establishmentId == 0) {
                        this.buttonEstablishment = false
                    } else {
                        this.buttonEstablishment = true
                    }
                } else
        ...
        beforeRouteLeave: function (to, from, next) {
            if (this.establishmentId == 0&&this.isdone == 1) {
                next(false)
                this.$confirm('您还未提交申请表，确定需要提出吗?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
