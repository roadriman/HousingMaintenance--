    ...
    <div class="flex-1 flex-y relative">
        ...
        <el-table :data="tableData" v-loading="loading" class="ft-table" style="width: 100%;" height="100%"
                ...
                <template slot-scope="scope">
                    <span>
                        ...
                        <el-button type="primary" icon="el-icon-edit" v-if="scope.row.establishment==0"
                            @click="openAddEstablishment(scope.row.id)">暂无</el-button>
                    </span>
                </template>
            </el-table-column>
                ...
                <template slot-scope="scope">
                    <span>
                        ...
                        <el-button type="primary" icon="el-icon-edit" v-if="scope.row.approval==0"
                            @click="openAddApproval(scope.row.id)">添加</el-button>

                    </span>
                    <span>
                        <el-tooltip class="item" effect="dark" content="这个项目不存在立项审批表！请尽快添加！" placement="right-start"
            ...
            <el-table-column label="操作" align="left">
                <template slot-scope="scope">
                    <div>
                        <el-button type="success" icon="el-icon-circle-plus" @click="finishApproval(scope.row.id)"
                            v-if="scope.row.status==4" :style="{'width':'90px','  margin-left':'0px'}">完成审计
                        </el-button>
                        <el-button type="danger" icon="el-icon-circle-plus" @click="rejectApproval(scope.row.id)"
                            v-if="scope.row.status==4" :style="{'width':'90px','  margin-left':'0px'}">退回概算
                        </el-button>
                    </div>
                </template>
            </el-table-column>
        ...
        <check-estimate @close="closeCheckEstimate" :id="id" v-if="CheckEstimate" :isupdate=true
            :source=this.source :projectId=this.projectId></check-estimate>
        <check-approval @close="closeCheckApproval" :id="id" v-if="CheckApproval" :isupdate=this.isupdate
            :source=this.source :projectId=this.projectId :name=$store.getters.name></check-approval>
...
<script>
    ...
    import {
        queryProjectListByMultipleStatus,
        deleteProject,
        updateProjectStatus
    } from '../service/http';
    export default {
        ...
        methods: {
            handleCurrentChange: async function (currentPage) {
                ...
                try {
                    ...
                    var res = await queryProjectListByMultipleStatus(
                        this.key,
                        "4,5",
                        currentPage,
                        this.pageSize,
            ...
            finishApproval: async function (id) {
                this.$confirm('此操作会将该项目完成审计并送至规划处, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(async () => {
            ...
            rejectApproval: async function (id) {
                this.$confirm('此操作会将该项目送至概算部门进行重新概算, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(async () => {
